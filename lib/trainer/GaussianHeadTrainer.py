...
class GaussianHeadTrainer():
    def __init__(self, dataloader, delta_poses, gaussianhead, supres, camera, optimizer, recorder, gpu_id):
        ...
        self.psnr_list = []
        #==================================SELF-DEFINED=========================================
        self.assign_clustering_ite = 0

    def train(self, start_epoch=0, epochs=1, compact_scale_rot = False, prune = False, compact_ite = 1):
        print("==================TOTAL EPOCHS: ",epochs,"========================")
        for epoch in range(start_epoch, epochs):
            psnr_list = []
            loss_list = []
            # =================SELF-DEFINED-COMPACT : BY JOU ================================
            if epoch == compact_ite:
                print("Initialing kmeans")
                self.gaussianhead.initializing_kmeans()
            # =================SELF-DEFINED-COMPACT : BY JOU ================================
            ...
            for idx, data in tqdm(enumerate(self.dataloader)):
                ...
                data = self.gaussianhead.generate(data, epoch)
                ...
                # save the last epoch's loss and psnr
                psnr_value = self.calculate_psnr(supres_images * cropped_visibles, cropped_images * cropped_visibles)
                ...
                #==========================COMPACT===========================
                self.gaussianhead.update_codebook()
                #============================================================
                log = {
                    'data': data,
                ...
                self.recorder.log(log,epoch)

            self.loss_list.append(np.mean(loss_list))
            print(sum(psnr_list) / len(psnr_list))
            self.psnr_list.append(sum(psnr_list) / len(psnr_list))
    ...
    def show_loss(self):
        print("LOSS: ",self.loss_list)
        print("PSNR: ",self.psnr_list)