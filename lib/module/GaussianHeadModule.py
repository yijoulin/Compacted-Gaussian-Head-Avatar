...
## Import k-means
from lib.compact3D.compact3D import compact_3D
...
class GaussianHeadModule(nn.Module):
    def __init__(self, cfg, xyz, feature, landmarks_3d_neutral, compact_epoch = 100, add_mouth_points=False):
        ...
        #=======================================SELF_PARAMETERS_INITIALISING=============================
        self.kmeans_scale_rot = compact_3D(num_clusters = 2000, num_iters = 5)
        self.scale_rot_id = None
        self.scale_rot_codebook = None
        self.kmeans_feature = compact_3D(num_clusters = 2000, num_iters = 5)
        self.feature_id = None
        self.feature_codebook = None
        #=======================================SELF_PARAMETERS_INITIALISING=============================

    def generate(self, data, epoch=0, prune = False):
        B = data['exp_coeff'].shape[0]
        #=====================forward_scale_rot==========================
        # 如果有compact的話，就拿quantized的scales去做後續forward並變成rendering及loss使用的scales
        if self.scale_rot_id != None:
            sampled_centers = torch.gather(self.scale_rot_codebook, 0, self.scale_rot_id.unsqueeze(-1).repeat(1, 7))
            self.scales = nn.Parameter(sampled_centers[:, :3])
            self.rotation = nn.Parameter(sampled_centers[:, 3:])

            sampled_centers_feature = torch.gather(self.feature_codebook, 0, self.feature_id.unsqueeze(-1).repeat(1, 128))
            self.feature = nn.Parameter(sampled_centers_feature)
        #=====================forward_scale_rot==========================
...
#==============================COMPACT===============================
#====================================================================
    def initializing_kmeans(self):
        self.kmeans_scale_rot.initializing("scale_rot")
        self.kmeans_scale_rot.forward_scale_rot(self.scales ,self.rotation, assign = True)
        self.kmeans_feature.initializing("feature")
        self.kmeans_feature.forward_feature(self.feature, assign = True)
        self.scale_rot_id = self.kmeans_scale_rot.get_nn_index()
        self.scale_rot_codebook = self.kmeans_scale_rot.get_centers()
        self.feature_id = self.kmeans_feature.get_nn_index()
        self.feature_codebook = self.kmeans_feature.get_centers()
        print("finish Initializing Kmeans")

    def update_codebook(self):
        if self.scale_rot_id != None:
            self.kmeans_scale_rot.forward_scale_rot(self.scales,self.rotation, assign = False)
            self.scale_rot_codebook = self.kmeans_scale_rot.get_centers()
            self.kmeans_feature.forward_feature(self.feature, assign = False)
            self.feature_codebook = self.kmeans_feature.get_centers()
    
    def state_dict(self,epoch):
        state_dict = super(GaussianHeadModule, self).state_dict()
        if epoch > 1:
            print("HOHOHO saving compacting result.")
            state_dict['scale_rot_id'] = self.kmeans_scale_rot.get_nn_index()
            state_dict['scale_rot_codebook'] = self.kmeans_scale_rot.get_centers()
            state_dict['feature_id'] = self.kmeans_feature.get_nn_index()
            state_dict['feature_codebook'] = self.kmeans_feature.get_centers()
            print(state_dict['feature_codebook'].size())

            if 'scales' in state_dict:
                del state_dict['scales']
                del state_dict['rotation']
                del state_dict['feature']
